...
namespace Hammock.AssetView.Platinum.Tools.RC.RelayClient.Models.Utils
{
    public static class Logger
    {
        ...
        static Logger()
        {
            ...
            {
                using (var subKey = baseKey.OpenSubKey(@"SOFTWARE\Hammock\PLATINUM\Client\RC", false))
                {
                    if (subKey != null)
                    {
            ...
            {
                using (var subKey = baseKey.OpenSubKey(@"SOFTWARE\Hammock\PLATINUM\Viewer", false))
                {
                    if (subKey != null)
                    {
                        _level = (Level)Convert.ToInt32(subKey.GetValue("LogLevel", "1"));
